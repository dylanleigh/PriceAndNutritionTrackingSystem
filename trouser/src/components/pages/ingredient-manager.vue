<template>
    <layout-default current-loc="ingredient_manager">
      ...
      <div class="header-all flex-row-between flex-gap-regular">
          ...
          <input-float id='ingredient_filter' label='Search' :extra='{onkeyup:"all_ingredients.gridOptions.api.refreshInfiniteCache()"}'></input-float>
      </div>
      ...
      <div class="ingredient">
          ...
          <form id="ingredient_edit_form" autocomplete="off">
              ...
              <input-float id='introduction' label='Introduction' hint="Introductory paragraph explains ingredient" :multiline="true" ></input-float>
                <div class="flex-row-equalfill">
                    <input-float id='name' label='Name' hint="Grilled Cheese" ></input-float>
                    <input-float id='slug' label='Slug' hint="grilled-cheese" input_mask_name="slug_mask" ></input-float>
                </div>

              <input-float id='description' label='Description' hint="A sandwich made with melted cheese" :multiline="true" ></input-float>
              <input-float id='notes' label='Notes' hint="Additional information about usage, types, etc." :multiline="true" ></input-float>
                <div class="flex-row-equalfill">
                    <input-float id='tags' label='Tags' hint="tag1,tag2,tag3" input_mask_name="tag_mask"></input-float>
                    <!-- You can't modify the owner, but I'm showing it here for this proof of concept -->
                    <input-float id='owner' label='Owner' hint="A sandwich made with melted cheese" :extra='{disabled: true, value:""}'></input-float>
                </div>
                <h3>Nutrition</h3>
                <div class="flex-row-equalfill">
                        <input-float id='serving' label='Serving Size (g)' hint="in grams" input_mask_name="nutrition_mask"></input-float>
                        <input-float id='kilojoules' label='kilojoules' input_mask_name=nutrition_mask></input-float>
                    </div>
                    <div class="flex-row-equalfill">
                        <input-float id='protein' label='protein (g)' hint="in grams" input_mask_name="nutrition_mask"></input-float>
                        <input-float id='carbohydrate' label='carbohydrate (g)' hint="in grams" input_mask_name="nutrition_mask"></input-float>
                        <input-float id='fat' label='fat (g)' hint="in grams" input_mask_name="nutrition_mask"></input-float>
                        <input-float id='saturatedfat' label='saturatedfat (g)' hint="in grams" input_mask_name="nutrition_mask"></input-float>
                    </div>
                    <div class="flex-row-equalfill">
                        <input-float id='sugar' label='sugar (g)' hint="in grams" input_mask_name="nutrition_mask"></input-float>
                        <input-float id='sodium' label='sodium (mg)' hint="in milligrams" input_mask_name="nutrition_mask"></input-float>
                        <input-float id='fibre' label='fibre (g)' hint="in grams" input_mask_name="nutrition_mask"></input-float>
                    </div>
            </form>
            <div class="flex-row-equalfill">
